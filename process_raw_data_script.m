...
mm_perf = process_raw_data(input_file, output_file, clang_info);

  % read gemm
input_file = strcat(raw_data_path,'gemm-ts.log');
output_file = strcat(data_path,'gemm.mat');
clang_info = [15, 25, 25, 0, 15, 15];
gemm_perf = process_raw_data(input_file, output_file, clang_info);
...
jacobi2d_int_perf = process_raw_data(input_file, output_file, clang_info);
